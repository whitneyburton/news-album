...
describe('fetchNatGeo', () => {
  const mockUrl = `https://newsapi.org/v2/top-headlines?sources=national-geographic&apiKey=${process.env.REACT_APP_NEWS_API_KEY}`;
  ...
  it('should dispatch setNatGeo with the articles', async () => {
    ...
    expect(mockDispatch).toHaveBeenCalledWith(
      setNatGeo(data.mockNatGeoArticlesObj.articles)
    );
  });
